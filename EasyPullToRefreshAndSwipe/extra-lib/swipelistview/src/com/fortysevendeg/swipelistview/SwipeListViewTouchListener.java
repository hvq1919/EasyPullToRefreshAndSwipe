...
public class SwipeListViewTouchListener implements View.OnTouchListener {
	...
	private static final int DEFAULT_DOWN_POSITION = -2;
	...
	private boolean mBIsRestrictedByOffset = false;
	...
	private int downPosition = DEFAULT_DOWN_POSITION;
	...
	private void setFrontView(View frontView) {
		...
		frontView.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (downPosition != ListView.INVALID_POSITION && downPosition != DEFAULT_DOWN_POSITION)
	...
	private void setBackView(View backView) {
		...
		backView.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				swipeListView.onClickBackView(downPosition, v);
			}
		});
	}
	...
	public boolean ismBIsRestrictedByOffset() {
		return mBIsRestrictedByOffset;
	}

	public void setmBIsRestrictedByOffset(boolean mBIsRestrictedByOffset) {
		this.mBIsRestrictedByOffset = mBIsRestrictedByOffset;
	}
	...
	protected void closeAnimate(int position) {
		closeAnimate(swipeListView.getChildAt(position - swipeListView.getFirstVisiblePosition()).findViewById(swipeFrontView),
				position);
	}
			...
			public void onAnimationEnd(Animator animation) {
				...
				if (swap) {
					...
					if (aux) {
						...
						swipeListView.onClosed(view, position, openedRight.get(position));
					}
				}
				resetCell();
			}
	...
	private void resetCell() {
		if (downPosition != ListView.INVALID_POSITION && downPosition != DEFAULT_DOWN_POSITION) {
			...
			Log.e("resetCell", "" + opened.get(downPosition));
	...
	public interface NLOnScrollListener {
		public void onScrollStateChanged(AbsListView absListView, int scrollState);

		public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount);
	}

	private NLOnScrollListener mNlOnScrollListener;
	private int lastedSelected = ListView.INVALID_POSITION;
		...
		return new AbsListView.OnScrollListener() {
			...
			public void onScrollStateChanged(AbsListView absListView, int scrollState) {
				...
				if (swipeClosesAllItemsWhenListMoves && scrollState == SCROLL_STATE_TOUCH_SCROLL
						&& swipeListView.getM_nScrollState() == SwipeListView.SCROLL_STATE_NORMAL) {
					closeOpenedItems();
				}
				...
				if (mNlOnScrollListener != null)
					mNlOnScrollListener.onScrollStateChanged(absListView, scrollState);
			}
			...
			public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
				...
				if (mNlOnScrollListener != null)
					mNlOnScrollListener.onScroll(view, firstVisibleItem, visibleItemCount, totalItemCount);
			}
		};
	}
	...
	/**
	 * Close all opened items except new one
	 * 
	 * @param position
	 *            of except item
	 */
	...
	public boolean onTouch(View view, MotionEvent motionEvent) {
		...
		case MotionEvent.ACTION_DOWN: {
			...
			for (int i = 0; i < childCount; i++) {
				...
				boolean allowSwipe = swipeListView.getAdapter().isEnabled(childPosition)
						&& (swipeListView.getAdapter().getItemViewType(childPosition) != AdapterView.ITEM_VIEW_TYPE_IGNORE && swipeListView
								.getAdapter().getItemViewType(childPosition) != AdapterView.ITEM_VIEW_TYPE_HEADER_OR_FOOTER);
				if(lastedSelected >= 0 && childPosition != lastedSelected && rect.contains(x, y))
					closeAnimate(lastedSelected);
				if (allowSwipe && rect.contains(x, y)) {
					...
					if (downPosition == DEFAULT_DOWN_POSITION || downPosition == ListView.INVALID_POSITION) {
						downPosition = childPosition;
						lastedSelected = downPosition;
						Log.e("onTouch", "" + lastedSelected);
					}

					else if (downPosition != DEFAULT_DOWN_POSITION && downPosition != ListView.INVALID_POSITION
							&& downPosition != childPosition) {
						downPosition = ListView.INVALID_POSITION;
						break;
					}
		...
		case MotionEvent.ACTION_UP: {
			if (velocityTracker == null || !swiping || downPosition == ListView.INVALID_POSITION) {
				if(lastedSelected != ListView.INVALID_POSITION) opened.set(lastedSelected, true);
				lastedSelected = ListView.INVALID_POSITION;
				break;
			}
		...
		case MotionEvent.ACTION_MOVE: {
			...
			try {
				if (swipeListView.getAdapter() instanceof SwipeListViewAdapter) {
					if (!opened.get(downPosition)) {
						if (((SwipeListViewAdapter) swipeListView.getAdapter()).getItemRestrictType(downPosition) == SwipeListView.DISABLE_SWIPE) {
							break;
						}
						if (deltaX > 0) { // Swipe right
							if (((SwipeListViewAdapter) swipeListView.getAdapter()).getItemRestrictType(downPosition) == SwipeListView.DISABLE_SWIPE_RIGHT) {
								break;
							}
						} else {// Swipe left
							if (((SwipeListViewAdapter) swipeListView.getAdapter()).getItemRestrictType(downPosition) == SwipeListView.DISABLE_SWIPE_LEFT) {
								break;
							}
						}
					}
				} else if ((((HeaderViewListAdapter) swipeListView.getAdapter()).getWrappedAdapter()) instanceof SwipeListViewAdapter)
					if (!opened.get(downPosition)) {
						if (((SwipeListViewAdapter) ((HeaderViewListAdapter) swipeListView.getAdapter()).getWrappedAdapter())
								.getItemRestrictType(downPosition) == SwipeListView.DISABLE_SWIPE) {
							break;
						}
						if (deltaX > 0) { // Swipe right
							if (((SwipeListViewAdapter) ((HeaderViewListAdapter) swipeListView.getAdapter())
									.getWrappedAdapter()).getItemRestrictType(downPosition) == SwipeListView.DISABLE_SWIPE_RIGHT) {
								break;
							}
						} else {// Swipe left
							if (((SwipeListViewAdapter) ((HeaderViewListAdapter) swipeListView.getAdapter())
									.getWrappedAdapter()).getItemRestrictType(downPosition) == SwipeListView.DISABLE_SWIPE_LEFT) {
								break;
							}
						}
					}
	...
	public void move(float deltaX) {
		...
		Log.d("swipeListView", "Delta " + deltaX + " Restricted Right offset " + (viewWidth - rightOffset) + " swipingRight "
				+ swipingRight);
		if (mBIsRestrictedByOffset) {
			if (swipingRight) {
				if (deltaX >= (viewWidth - rightOffset))
					return;
			} else {
				if (deltaX <= (leftOffset - viewWidth))
					return;
			}
		}
		...
		if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_DISMISS) {
			...
			if ((swipingRight && deltaX > 0 && posX < DISPLACE_CHOICE)
					|| (!swipingRight && deltaX < 0 && posX > -DISPLACE_CHOICE) || (swipingRight && deltaX < DISPLACE_CHOICE)
	...
	public void setmNlOnScrollListener(NLOnScrollListener mNlOnScrollListener) {
		this.mNlOnScrollListener = mNlOnScrollListener;
	}
