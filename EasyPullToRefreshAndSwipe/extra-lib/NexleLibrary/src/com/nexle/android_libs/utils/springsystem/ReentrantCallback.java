...
/**
 * Because Java doesn't permit additions/removals to a list while iterating, a callback cannot remove itself when it is
 * being called.
 *
 * This class makes it safe for Callbacks to remove themselves. Also, it is thread-safe.
 *
 * Implementation detail: This class works by storing an inner set that will be modified by the calls to
 * add/removeListener. When getListeners() or iterator() is called, an immutable copy from the inner set is returned.
 * That copy will be reused if no changes have been done to the set since last request for it.
 */
