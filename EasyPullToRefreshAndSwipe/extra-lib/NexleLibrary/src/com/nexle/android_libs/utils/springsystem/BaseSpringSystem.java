...
/**
 * BaseSpringSystem maintains the set of springs within an Application context. It is responsible for Running the spring
 * integration loop and maintains a registry of all the Springs it solves for. In addition to listening to physics
 * events on the individual Springs in the system, listeners can be added to the BaseSpringSystem itself to provide pre
 * and post integration setup.
 */
public class BaseSpringSystem {
	 ...
	 * @param springLooper
	 *            parameterized springLooper to allow testability of the physics loop
	 */
	...
	/**
	 * Deregisters a Spring from this BaseSpringSystem, so it won't be iterated anymore. The Spring should not be used
	 * anymore after doing this.
	 *
	 * @param spring
	 *            the Spring to deregister
	 */
	...
	/**
	 * This is used internally by the {@link Spring}s created by this {@link BaseSpringSystem} to notify it has reached
	 * a state where it needs to be iterated. This will add the spring to the list of active springs on this system and
	 * start the iteration if the system was idle before this call.
	 * 
	 * @param springId
	 *            the id of the Spring to be activated
	 */
