...
package com.nexle.android_libs.utils.springsystem;
...
public class Spring {
	...
	/**
	 * Destroys this Spring, meaning that it will be deregistered from its BaseSpringSystem so it won't be iterated
	 * anymore and will clear its set of listeners. Do not use the Spring after calling this, doing so may just cause an
	...
	/**
	 * Set the displaced value to determine the displacement for the spring from the rest value. This value is retained
	 * and used to calculate the displacement ratio. This also updates the start value of the Spring.
	 * 
	 * @param currentValue
	...
	/**
	 * advance the physics simulation in SOLVER_TIMESTEP_SEC sized chunks to fulfill the required realTimeDelta. The
	 * math is inlined inside the loop since it made a huge performance impact when there are several springs being
	 * advanced.
	 * 
	 * @param time
	 *            clock time
	...
	void advance(double time, double realDeltaTime) {
		...
		if (isAtRest && mWasAtRest) {
			/*
			 * begin debug Log.d(TAG, "bailing out because we are at rest:" + getName()); end debug
			 */
			return;
		}
		...
		/*
		 * begin debug long startTime = System.currentTimeMillis(); int iterations = 0; end debug
		 */
		...
		while (mTimeAccumulator >= SOLVER_TIMESTEP_SEC) {
			/*
			 * begin debug iterations++; end debug
			 */
		...
		/*
		 * begin debug long endTime = System.currentTimeMillis(); long elapsedMillis = endTime - startTime; Log.d(TAG,
		 * "iterations:" + iterations + " iterationTime:" + elapsedMillis + " position:" + mCurrentState.position +
		 * " velocity:" + mCurrentState.velocity + " realDeltaTime:" + realDeltaTime + " adjustedDeltaTime:" +
		 * adjustedDeltaTime + " isAtRest:" + isAtRest + " wasAtRest:" + mWasAtRest); end debug
		 */
	...
	/**
	 * Check if this spring should be advanced by the system. * The rule is if the spring is currently at rest and it
	 * was at rest in the previous advance, the system can skip this spring
	 * 
	...
	/**
	 * Check if the spring was at rest in the prior iteration. This is used for ensuring the ending callbacks are fired
	 * as the spring comes to a rest.
	 * 
	...
	/**
	 * linear interpolation between the previous and current physics state based on the amount of timestep remaining
	 * after processing the rendering delta time in timestep sized chunks.
	 * 
	 * @param alpha
	 *            from 0 to 1, where 0 is the previous state, 1 is the current state
	 */
	...
	/**
	 * This method checks to see that the current spring displacement value is equal to the input, accounting for the
	 * spring's rest displacement threshold.
	 * 
	 * @param value
	 *            The value to compare the spring value to
	 * @return Whether the displacement value from the spring is within the bounds of the compare value, accounting for
	 *         threshold
	 */
